...
use super::paged_kv::{PagedKvCache, BLOCK_BYTES};
...
impl RuntimeFragmentationTracker {
    ...
    pub fn record(&mut self, cache: &PagedKvCache) {
        ...
        let _blocks_per_sb = cache.blocks_per_superblock();

        let blocks_not_free = total_blocks - free_blocks;

        let memory_allocated_not_free = blocks_not_free * BLOCK_BYTES;
    ...
    /// Return all recorded samples for analysis.

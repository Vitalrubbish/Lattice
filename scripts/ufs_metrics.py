#!/usr/bin/env python3
"""
ufs_metrics.py — Unified Fragmentation Standard (UFS) for KV cache benchmarks.

Provides the same 4 fragmentation metrics used by the Rust baseline,
so vLLM benchmarks produce directly comparable results.
...
def compute_metrics(
    *,
    ...
    For vLLM (PyTorch allocator — no superblocks):
      block_bytes = kv_heads × head_dim × sizeof(f16) × block_size
      actual_physical_bytes = total_blocks_allocated × block_bytes × num_layers × 2
      actual_active_bytes   = blocks_in_use × block_bytes × num_layers × 2
...
def write_frag_csv(samples: List[UnifiedFragMetrics], path: str):
    """Write fragmentation time-series to CSV (compatible with baseline format)."""
    import csv
    ...
    print(f"Wrote {len(samples)} frag samples to {path}", file=__import__('sys').stderr)

...
`run_kcmm_integration_bench.sh` 是 KCMM §1.6 引擎集成基准测试的运行脚本。该测试将 `NaiveTransformer` 模型与 `KvCacheBackend` 缓存后端结合，在模拟的连续批处理（continuous batching）工作负载下运行，对比 **PagedKvCache（基线，无分级存储）** 和 **KcmmPool（分级存储开启）** 两种后端的性能表现。
...
| **容量比率（capacity ratio）** | KCMM 完成的请求数 / 基线完成的请求数 |
| **吞吐量比率（throughput ratio）** | KCMM 吞吐量 / 基线吞吐量 |
| **每步延迟分布** | 每个 forward step 的 P50/P90/P95/P99 延迟（µs） |
| **每步延迟开销** | KCMM 相比基线的 P50/P99 延迟增加百分比 |
...
KCMM 分级存储在相同 GPU 内存预算下，在内存压力场景中实现 ≥ 1.3× 的吞吐量/容量提升（相比无分级存储基线）。
...
使用 **TinyLlama** 参数规模进行测试：

| 参数 | 值 |
|------|-----|
| 层数（num_hidden_layers） | 22 |
| KV 头数（num_key_value_heads） | 4 |
| 头维度（head_dim） | 64 |
| 隐藏维度（hidden_size） | 2048 |
...
1. **冷却周期**（仅 KCMM 模式）：每 8 步将约 1/4 活跃序列标记为"冷却"（cool），为逐出创建候选；其余活跃序列重新 touch 保持热度。
2. **序列增长**：遍历所有活跃序列，为需要扩展的序列分配新块。若分配失败：
   - **KCMM 模式**：触发批量逐出（每次至少 8 个候选块），然后重试分配。若逐出前需检查块是否处于 `CpuResident` 状态，若是则批量恢复。
   - **基线模式**：直接截断该序列。
...
| total_requests | 32 |
| arrival_interval | 12 |
| GPU 内存预算 | ~110 MiB（640 块 × 176 KiB/块） |

**输出详情**：
- 基线与 KCMM 的完整指标对比表格（含完成数、总 token 数、decode token 数、耗时、吞吐量、峰值并发数）
...
| 测试对象 | KcmmPool 底层 API | NaiveTransformer + KvCacheBackend |
